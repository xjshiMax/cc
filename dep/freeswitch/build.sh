...
if [ ! -f "$FS_FILE" ]; then    
    echo "can't find $FS_FILE, build failed";
    exit 1;
fi

echo "tar xzf $FS_FILE..."
tar xzf $FS_FILE
...
cp ../../../platform/fw-mod/ivrloadbalance/ src/mod/applications/mod_ivr_loadbalance/

patch_files=`ls /r/patch/*.patch`
if [ $? = 0 ] ; then
