...
// for namespace bgcc2
#include "conf_unit.h"
//#include "./internal/bgcc_constraint_unit.h"
//#include "./internal/bgcc_constraint.h"
//#include "./internal/bgcc_constraint_parser.h"
#include "./internal/bgcc_configure_parser.h"
#include "exception.h"
// end for namespace bgcc2
...
bool ConfLoader::load_conf(const std::string& conf_file, ConfUnit& conf_unit)
{
    ...
    if (!conf_unit.is_group()) {
        std::cerr << "error: load_conf: The 3rd argument need to be (bgcc2::ConfUnit::UT_GROUP)" << std::endl;
        return false;
    }

    try {
        ConfigureParser conf_parser;
        conf_parser.parse(conf_file, conf_unit);
    } catch (bgcc::ConfigurationException& e) {
