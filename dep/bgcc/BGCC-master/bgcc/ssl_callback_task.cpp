...
bgcc::SSLCallBackTask::SSLCallBackTask(const std::string server, int32_t port,
		const std::string proxy, ServiceManager *sm, Selector *selector):
	_server(server), _port(port), _prx(proxy), _sm(sm),
	_pselector(selector){
			_connect = SharedPointer<SSLClientSocket>(new(std::nothrow) SSLClientSocket(_server, _port));
...
bgcc::SSLCallBackTask::~SSLCallBackTask(){
}
...
bool bgcc::SSLCallBackTask::Register(){
	bool ret=false;
...
const int32_t bgcc::SSLCallBackTask::DEFAULT_WAIT_TIMEOUT=200;
